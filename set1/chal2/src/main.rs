...
fn main() {
    ...
    let decoded_hex1 = decode(hex1).expect("Invalid hex string");
    let decoded_hex2 = decode(hex2).expect("Invalid hex string");
    let xor = encode(xor(&decoded_hex1, &decoded_hex2));
